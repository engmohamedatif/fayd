...
import { cleanText, extOf, objectUrlOf } from "@/lib/media";
...
export function DocumentViewer({ url, name }: Props) {
    ...
    const load = async () => {
      try {
        if (ext === "txt") {
          ...
          const value = await objectUrlOf(url);
          if (!alive) return URL.revokeObjectURL(value);
          setObjectUrl(value);
          cleanup = () => URL.revokeObjectURL(value);
  ...
  return (
    <div className="relative min-h-[75dvh] h-full bg-background">
      ...
      {ext === "pdf" && objectUrl && <object data={objectUrl} type="application/pdf" className="h-full min-h-[75dvh] w-full" />}

...
export const Route = createFileRoute("/quran-read")({
  head: () => ({ meta: [{ title: "قراءة القرآن الكريم - فيض" }, { name: "description", content: "قراءة القرآن الكريم كاملاً برسم المصحف." }] }),
  component: Layout,
});
...
function Layout() {
  ...
  const isChild = matches.some((m) => m.routeId === "/quran-read/$surah");
...
function SurahList() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8 space-y-6">
      <h1 className="text-2xl md:text-3xl font-extrabold text-center">قراءة القرآن الكريم</h1>
      <p className="text-center text-sm text-muted-foreground">اختر السورة للقراءة</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
        {filtered.map((s) => (
          <Link key={s.number} to="/quran-read/$surah" params={{ surah: String(s.number) }} className="flex items-center gap-3 rounded-2xl border border-border p-4 md:hover:bg-foreground md:hover:text-background transition">
